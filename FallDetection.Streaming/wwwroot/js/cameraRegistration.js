...
const CameraRegistration = {
    ...
    async approveRegistration() {
        ...
        if (!cameraName) {
            NotificationSystem.show('Please enter a camera name.', 'warning');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                NotificationSystem.show(`✅ Camera registered as: ${cameraName}`, 'success');
            } else {
                const errorData = await response.json().catch(() => ({}));
                NotificationSystem.show(`❌ Registration failed: ${errorData.error || 'Unknown error'}`, 'error');
            }
        } catch (error) {
            console.error('Registration error:', error);
            NotificationSystem.show('❌ Registration error.', 'error');
        }
    },
